...
exports.handler = JWEValidator(async function (context, event, callback) {
  ...
  async function PluginTaskUpdate(type, taskSid, attr, state) {
    console.log(
      'type: ' + type + '  taskSid: ' + taskSid + '  state: ' + state
    );
    ...
    let tsk = await client.taskrouter
  ...
  switch (mode) {
    ...
    case 'requeueTasks':
      ...
      async function completeTask(taskSid) {
        try {
          let completedTask = await client.taskrouter
            .workspaces(context.TWILIO_WORKSPACE_SID)
            .tasks(taskSid)
            .update({
      ...
      async function requeueMain() {
        ...
        //  create new task
        let updTask = await PluginTaskUpdate(
          event.type,
          event.taskSid,
          event.attributes,
          event.state
        );
        let newTsk = await newTask(event.workflowSid, newAttributes);
